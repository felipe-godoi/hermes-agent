...
| `pre_gateway_dispatch` | Directive/control | Incoming non-internal message before auth/pairing/dispatch; first valid `skip`, `rewrite`, `allow`, or `delegate` controls flow. | `event`, `gateway`, `session_store` | Extremely privileged in-process objects expose inbound user/routing data and host handles. |
...
| `{"action": "delegate"}` | Skip **only** the `_is_user_authorized()` check for this one message, then continue normal dispatch (session creation, toolset resolution, agent turn, reply delivery) exactly as if the sender had passed auth. The platform's own allowlist (`WHATSAPP_ALLOWED_USERS`, etc.) and `_is_user_authorized()` itself are untouched for every other sender and every other call site — this is a single-message, single-call-site bypass a plugin must re-earn on every inbound message by independently re-validating its own grant (e.g. an explicit, TTL'd, per-contact delegation record). Combine with a `toolsets_for_source()` adapter override (see [Adding Platform Adapters](/developer-guide/adding-platform-adapters)) to keep the resulting session restricted to a minimal toolset, since `delegate` only affects authorization, not tool access. |

**Use cases:** Listen-only group chats (only respond when tagged; buffer ambient messages into context); human handover (silent-ingest customer messages while owner handles the chat manually); per-profile rate limiting; policy-driven routing; time-boxed delegated conversations with an otherwise-unauthorized contact (see the bundled `whatsapp-delegation` plugin at `plugins/whatsapp_delegation/` for a full worked example, including the matching `toolsets_for_source` and `pre_tool_call` restrictions).
